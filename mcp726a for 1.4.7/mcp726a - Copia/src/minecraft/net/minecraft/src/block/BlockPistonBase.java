...
public class BlockPistonBase extends Block {
     ...
     * flag. This will actually never get
     * called by TileEntityRendererPiston.renderPiston() because
     * TileEntityPiston.shouldRenderHead() will always return
     * false.
     */
    ...
    public int getBlockTextureFromSideAndMetadata(int par1, int par2) {
        ...
        return var3 > 5 ? this.blockIndexInTexture
                : (par1 == var3
                        ? (!isExtended(par2) && this.minX <= 0.0D && this.minY <= 0.0D && this.minZ <= 0.0D
                                && this.maxX >= 1.0D && this.maxY >= 1.0D && this.maxZ >= 1.0D
                                        ? this.blockIndexInTexture
                                        : 110)
                        : (par1 == Facing.faceToSide[var3] ? 109 : 108));
    }
    ...
    private boolean isIndirectlyPowered(World par1World, int par2, int par3, int par4, int par5) {
        return par5 != 0 && par1World.isBlockIndirectlyProvidingPowerTo(par2, par3 - 1, par4, 0) ? true
                : (par5 != 1 && par1World.isBlockIndirectlyProvidingPowerTo(par2, par3 + 1, par4, 1) ? true
                        : (par5 != 2 && par1World.isBlockIndirectlyProvidingPowerTo(par2, par3, par4 - 1, 2) ? true
                                : (par5 != 3 && par1World.isBlockIndirectlyProvidingPowerTo(par2, par3, par4 + 1, 3)
                                        ? true
                                        : (par5 != 5
                                                && par1World.isBlockIndirectlyProvidingPowerTo(par2 + 1, par3, par4, 5)
                                                        ? true
                                                        : (par5 != 4 && par1World.isBlockIndirectlyProvidingPowerTo(
                                                                par2 - 1, par3, par4, 4)
                                                                        ? true
                                                                        : (par1World.isBlockIndirectlyProvidingPowerTo(
                                                                                par2, par3, par4, 0)
                                                                                        ? true
                                                                                        : (par1World
    ...
    public void onBlockEventReceived(World par1World, int par2, int par3, int par4, int par5, int par6) {
        ...
        if (par5 == 0) {
            ...
            TileEntity var7 = par1World.getBlockTileEntity(par2 + Facing.offsetsXForSide[par6],
            ...
            if (this.isSticky) {
                ...
                if (!var13 && var11 > 0 && canPushBlock(var11, par1World, var8, var9, var10, false)
                    ...
                    par1World.setBlockWithNotify(par2 + Facing.offsetsXForSide[par6],
                            par3 + Facing.offsetsYForSide[par6], par4 + Facing.offsetsZForSide[par6], 0);
                }
            } else {
                par1World.setBlockWithNotify(par2 + Facing.offsetsXForSide[par6], par3 + Facing.offsetsYForSide[par6],
        ...
        while (true) {
            ...
            while (var6 != par2 || var7 != par3 || var8 != par4) {
                ...
                if (var12 == this.blockID && var9 == par2 && var10 == par3 && var11 == par4) {
                    par1World.setBlockAndMetadataWithUpdate(var6, var7, var8, Block.pistonMoving.blockID,
                            par5 | (this.isSticky ? 8 : 0), false);
                    ...
                    par1World.setBlockTileEntity(var6, var7, var8,
                            BlockPistonMoving.getTileEntity(var12, var13, par5, true, false));
                }
