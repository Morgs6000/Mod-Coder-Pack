...
public class gv extends ik
{
    ...
    public gv(int i)
    {
        field_867_a = ly.field_424_aX.field_376_bc;
        field_866_b = i;
    }

    public boolean func_516_a(cn cn1, Random random, int i, int j, int k)
    {
        if(cn1.func_599_f(i, j, k) != gb.field_1332_f)
        {
            return false;
        }
        float f = random.nextFloat() * 3.141593F;
        double d = (float)(i + 8) + (eo.func_1106_a(f) * (float)field_866_b) / 8F;
        double d1 = (float)(i + 8) - (eo.func_1106_a(f) * (float)field_866_b) / 8F;
        double d2 = (float)(k + 8) + (eo.func_1114_b(f) * (float)field_866_b) / 8F;
        double d3 = (float)(k + 8) - (eo.func_1114_b(f) * (float)field_866_b) / 8F;
        ...
        for(int l = 0; l <= field_866_b; l++)
        {
            ...
            double d10 = (double)(eo.func_1106_a(((float)l * 3.141593F) / (float)field_866_b) + 1.0F) * d9 + 1.0D;
            double d11 = (double)(eo.func_1106_a(((float)l * 3.141593F) / (float)field_866_b) + 1.0F) * d9 + 1.0D;
            for(int i1 = (int)(d6 - d10 / 2D); i1 <= (int)(d6 + d10 / 2D); i1++)
            {
                for(int j1 = (int)(d7 - d11 / 2D); j1 <= (int)(d7 + d11 / 2D); j1++)
                {
                    for(int k1 = (int)(d8 - d10 / 2D); k1 <= (int)(d8 + d10 / 2D); k1++)
                    {
                        ...
                        int l1 = cn1.func_600_a(i1, j1, k1);
                        if(l1 == ly.field_393_F.field_376_bc)
                        {
                            cn1.func_634_a(i1, j1, k1, field_867_a);
                        }
                    }
