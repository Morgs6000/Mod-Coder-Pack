...
public class f extends bc
{
    ...
    public boolean a(dy dy1, java.util.Random random, int i, int j, int k)
    {
        if(dy1.c(i, j, k) != iq.f)
            return false;
        float f1 = random.nextFloat() * 3.141593F;
        double d = (float)(i + 8) + (gj.a(f1) * (float)b) / 8F;
        double d1 = (float)(i + 8) - (gj.a(f1) * (float)b) / 8F;
        double d2 = (float)(k + 8) + (gj.b(f1) * (float)b) / 8F;
        double d3 = (float)(k + 8) - (gj.b(f1) * (float)b) / 8F;
