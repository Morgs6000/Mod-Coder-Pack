...
public class BlockRedstoneTorch extends BlockTorch
{
    ...
    public int func_232_a(int i, int j)
    {
        if(i == 1)
        {
            return Block.redstoneWire.func_232_a(i, j);
        } else
        {
    ...
    private boolean func_273_a(World world, int i, int j, int k, boolean flag)
    {
        if(flag)
        {
            field_450_b.add(new RedstoneUpdateInfo(i, j, k, world.worldTime));
        }
        int l = 0;
    ...
    {
        boolean flag = func_272_h(world, i, j, k);
        for(; field_450_b.size() > 0 && world.worldTime - ((RedstoneUpdateInfo)field_450_b.get(0)).field_1010_d > 100L; field_450_b.remove(0)) { }
        if(field_451_a)
        {
            if(flag)
            {
                world.func_688_b(i, j, k, Block.torchRedstoneIdle.blockID, world.func_602_e(i, j, k));
        ...
        {
            world.func_688_b(i, j, k, Block.torchRedstoneActive.blockID, world.func_602_e(i, j, k));
        }
    }
